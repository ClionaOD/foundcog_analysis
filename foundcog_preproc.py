...
with open(
    os.path.abspath(path.join(reference_files_path, "flirt_dof_manualselection.json")),
    "rt",
) as f:
...
func_file = path.join(
    ...
    "ses-{session}",
    "func",
    "sub-{subject_id}_ses-{session}_task-{task_name}_dir-AP_run-{run}_bold.nii.gz",
)
fmap_file = path.join(
    "sub-{subject_id}",
    "ses-{session}",
    "fmap",
    "sub-{subject_id}_ses-{session}_dir-{dir}_run-???_epi.nii.gz",
)
event_file = path.join(
    "sub-{subject_id}",
    "ses-{session}",
    "func",
    "sub-{subject_id}_ses-{session}_task-{task_name}_dir-AP_run-{run}_events.tsv",
)
...
for sub in subject_list:
    ...
    for dir in ["AP", "PA"]:
        ...
        if not fmap_files:
            print(
                f"Warning no fieldmaps found for {sub} phase-encoding direction {dir}"
            )
...
for sub, sub_items in iter_items.items():
    ...
    infosource = Node(
        IdentityInterface(fields=["session", "task_name", "run"]), name="infosource"
    )
    ...
    datasink_run = Node(
        DataSink(base_directory=experiment_dir, container=output_dir),
        name="datasink_run",
    )
    ...
    def select_by_session(
        ...
        return (
            applytopup_method[sessind],
            out_fieldcoef[sessind],
            out_movpar[sessind],
            out_enc_file[sessind],
            out_corrected[sessind],
        )
    ...
    if are_fmaps:
        ...
        bg_fmap_files_node = Node(
            Function(
                function=bg_fmap_files,
                input_names=[
                    "experiment_dir",
                    "database_path",
                    "subject_id",
        ...
        mean_fmaps = MapNode(
            fsl.maths.MeanImage(), iterfield="in_file", name="mean_fmaps"
        )
    ...
    def select_fmaps(in_files):
        ...
        for fmap in in_files:
            ...
            with open(fmap_s[0] + ".json", "r") as f:
                ...
                encoding_direction[ah].append(
                    remap[fmap_json["PhaseEncodingDirection"]]
                )
        ...
        if len(ahs[longest_key]) < 2:
            ...
            encoding_direction = {
                "all": [x for k, v in encoding_direction.items() for x in v]
            }
        ...
        print(
            f"Affines {ahs} longest is {longest_key} encoding directions {encoding_direction[longest_key]} readout times {readout_times[longest_key]}"
        )
        return (
            ahs[longest_key],
            encoding_direction[longest_key],
            readout_times[longest_key],
            applytopup_method,
        )

    select_fmaps_node = Node(
        Function(
            function=select_fmaps,
    ...
    get_coreg_reference_node = Node(
        Function(
            function=get_coreg_reference,
            input_names=["in_files"],
            output_names=["reference"],
        ),
        name="get_coreg_reference",
    )
    ...
    coreg_singleband_to_submean = Node(
        fsl.FLIRT(dof=6), name="coreg_singleband_to_submean"
    )
    ...
    flirt_to_template = Node(
        fsl.FLIRT(
            bins=640,
            cost_func="mutualinfo",
            reference=template,
            in_matrix_file=input_matrix_file,
        ),
    ...
    if are_fmaps:
        flirt_to_template_singleband = Node(
            fsl.FLIRT(
                bins=640,
                cost_func="mutualinfo",
                reference=template,
    ...
    if flirt_dof_bysub[sub]:
        flirt_to_template_manualselection = Node(
            fsl.FLIRT(
                bins=640,
                dof=flirt_dof_bysub[sub][0],
                cost_func="mutualinfo",
                reference=template,
                in_matrix_file=input_matrix_file,
            ),
        ...
        flirt_fig_node_manualselection = Node(
            Function(
                function=flirt_fig,
                input_names=["in_file", "template"],
                output_names="out_file",
            ),
            name=f"flirt_fig_manualselection",
        )
        flirt_fig_node_manualselection.inputs.template = template

        combine_xfms_manual_selection = Node(
            fsl.utils.ConvertXFM(concat_xfm=True), name=f"combine_xfms_manual_selection"
        )

        normalize_epi = Node(
            fsl.preprocess.ApplyXFM(reference=template_for_norm_2mm, apply_xfm=True),
            name=f"normalization_manualselection",
        )
        ...
        normalize_snr = Node(
            fsl.preprocess.ApplyXFM(reference=template_for_norm_2mm, apply_xfm=True),
            name=f"normalize_snr",
        )

    # Niworkflows SignalExtraction for ROI Extraction
    roi_extraction = Node(
        SignalExtraction(label_files=schaefer_roi, class_labels=schaefer_roi_labels),
        name=f"roi_extraction",
    )
    ...
    # Calculate globals
    globals_node = Node(
        Function(function=get_globals, input_names="in_file", output_names="out_file"),
        name=f"out_file",
    )
    ...
    if are_fmaps:
        ...
        preproc.connect(
            select_fmaps_node,
            "applytopup_method",
            select_by_session_node,
            "applytopup_method",
        )

        # Pass this to applytopup
        preproc.connect(
            select_by_session_node, "out_fieldcoef", applytopup, "in_topup_fieldcoef"
        )
        preproc.connect(
            select_by_session_node, "out_movpar", applytopup, "in_topup_movpar"
        )
        preproc.connect(
            select_by_session_node, "out_enc_file", applytopup, "encoding_file"
        )
        preproc.connect(
            select_by_session_node, "applytopup_method", applytopup, "method"
        )
    ...
    preproc.connect(
        get_coreg_reference_node, "reference", apply_xfm_to_snr, "reference"
    )
    ...
    preproc.connect(
        get_coreg_reference_node, "reference", apply_xfm_to_mean, "reference"
    )
    ...
    if are_fmaps:
        preproc.connect(
            select_by_session_node,
            "out_corrected",
            coreg_singleband_to_submean,
            "in_file",
        )
        preproc.connect(
            submean, "output_average_image", coreg_singleband_to_submean, "reference"
        )
    ...
    preproc.connect(
        ...
        "out_matrix_file",
        datasink_dof,
        f"submean_affineflirt_matrix",
    )
    preproc.connect(flirt_to_template, "out_file", flirt_fig_node, "in_file")
    preproc.connect(
        flirt_fig_node, "out_file", datasink_dof, f"submean_affineflirt_figure"
    )
    ...
    if flirt_dof_bysub[sub]:
        ...
        preproc.connect(
            ...
            flirt_to_template_manualselection,
            "in_file",
        )
        preproc.connect(
            flirt_to_template_manualselection,
            "out_file",
            datasink_subj,
            f"submean_affineflirt_manualselection",
        )
        preproc.connect(
            flirt_to_template_manualselection,
            "out_matrix_file",
            datasink_subj,
            f"submean_affineflirt_matrix_manualselection",
        )
        preproc.connect(
            flirt_to_template_manualselection,
            "out_file",
            flirt_fig_node_manualselection,
            "in_file",
        )
        preproc.connect(
            flirt_fig_node_manualselection,
            "out_file",
            datasink_subj,
            f"submean_affineflirt_figure_manualselection",
        )

        # Combine coreg of run to submean and normalisation derived from EPIs into a single transform
        preproc.connect(
            coreg_runs, "out_matrix_file", combine_xfms_manual_selection, "in_file"
        )
        preproc.connect(
            flirt_to_template_manualselection,
            "out_matrix_file",
        ...
        preproc.connect(
            combine_xfms_manual_selection, "out_file", normalize_snr, "in_matrix_file"
        )

        preproc.connect(
            normalize_snr, "out_file", datasink_run, "snr_normalized_to_common_space"
        )  # and send to datasink
        ...
        preproc.connect(
            combine_xfms_manual_selection, "out_file", normalize_epi, "in_matrix_file"
        )

        preproc.connect(
            normalize_epi, "out_file", datasink_run, "normalized_to_common_space"
        )  # and send to datasink
        ...
        print(
            f"No dof selected for sub {sub} yet - Please select flirt dof and re-run the pipeline"
        )
