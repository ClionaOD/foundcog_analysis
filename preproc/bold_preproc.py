...
def get_wf_bold_preproc(experiment_dir, working_dir, output_dir):
    ...
    outputnode = Node(
        IdentityInterface(fields=["func", "par_file", "fwd_file"]), name="outputnode"
    )
    ...
    def getreferencevolume(func, referencetype="standard"):
        ...
        if referencetype == "standard":
            ...
            raise NotImplementedError(
                f"Reference type '{referencetype}' is not implemented."
            )
    ...
    #  MCFLIRT
    motion_correct = Node(
        fsl.MCFLIRT(save_plots=True, output_type="NIFTI"), name="mcflirt"
    )
    ...
    join_plot_motion = JoinNode(
        IdentityInterface(fields=["out_file"]),
        name="join_plot_motion",
        joinfield="out_file",
        joinsource="plot_motion",
    )

    # Normalize motion from FSL to SPM format
    normalize_motion = Node(
        NormalizeMotionParams(format="FSL"), name="normalize_motion"
    )

    calc_fwd = Node(
        interface=ni_confounds.FramewiseDisplacement(
            parameter_source="SPM"
        ),  # use parameter source as SPM because these have been processed with NormalizeMotionParams, for compatability with fmriprep
        name="calc_fwd",
    )
    ...
    def plot_bad_volumes(in_file, fwd, nimgs=10):
        ...
        fwd = np.insert(
            fwd, 0, np.nan
        )  # fwd is a difference between two images. We'll use the fwd to index the second of these (likely more artifacts due to spin history)
